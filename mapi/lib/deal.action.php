...
class dealModule
{
    ...
    public function pack_prop($table,$video_redis,$total_diamonds,$total_score,$total_ticket,$num,$prop,$is_plus,$video_id,$user_id,$prop_id,$podcast_id,$group_id,$room_type,$from,$robot_diamonds){
        ...
        try
        {
            ...
            $user_redis = new UserRedisService();
            //是否全局飘屏
            if($prop['diamonds']>=app_conf('GLOBAL_GIFT_DIAMONDS')&&$prop['is_red_envelope'] ==0)
            {
                $global_gift=true;
            }else{
                $global_gift=false;
            ...
            if($total_diamonds==0&&$total_score==0&&$total_ticket==0){
                ...
                if($GLOBALS['db']->affected_rows()){
                    ...
                    $video_prop['family_id'] = $user_redis->getOne_db($podcast_id,'family_id');
                    ...
                    //将礼物写入mysql表中
                    $field_arr = array('prop_id','prop_name','is_red_envelope', 'total_score', 'total_diamonds', 'total_ticket','from_user_id', 'to_user_id', 'create_time','create_date','num','video_id','group_id','create_ym','create_d','create_w','from_ip','family_id');
                    ...
                    */
                    ...
                    if ($prop['is_red_envelope'] == 1){
                        ...
                        $desc = "送了".$num."个".$prop['name'];
                        //礼物接收人（主播）收到的提示内容
                        $desc2 = '收到'.$num."个".$prop['name'];
                    }
                    ...
                    $ext['is_much'] = $prop['is_much'];//1:可以连续发送多个;用于小金额礼物
                    $ext['room_id'] = $video_id;//直播ID 也是room_id;只有与当前房间相同时，收到消息才响应
                    $ext['to_user_name'] = $user_redis->getOne_db($podcast_id,'nick_name');
                    ...
                    if($global_gift){
                        $ext['global_gift']=1;
                        $ext['video']=$video_redis->getRow_db($video_id);
                    }else{
                        $ext['global_gift']=0;
                    }
                    ...
                    if (isset($_REQUEST['is_debug'])){
                        ...
                        if(!$global_gift)
                        {
                            ...
                            $groups_ret= $api->group_get_appid_group_list2(10000);
                            if($groups_ret['ActionStatus'] == 'FAIL'&&$groups_ret['ErrorCode'])
                            {
                                $groups_ret= $api->group_get_appid_group_list2(10000);
                            }
                            $groups=$groups_ret['GroupIdList'];
                            foreach($groups as $group)
                            {
                                $group_id=$group['GroupId'];
                                $live_in=$video_redis->getOne_db(intval($group_id), 'live_in');
                                if($live_in)
                                {
                                    $ret = $api->group_send_group_msg2($user_id, $group_id, $msg_content);
                                    if ($ret['ActionStatus'] == 'FAIL' && $ret['ErrorCode'] == 10002){
                                    ...
                                    $api->group_destroy_group( $group_id );
                                }
                            }
                        }
            ...
            try {
                ...
                if ($total_diamonds == 0 && $total_score == 0 && $total_ticket == 0) {
                    ...
                    if ($GLOBALS['db']->affected_rows()) {
                        ...
                        $video_prop['create_w'] = to_date($video_prop['create_time'], 'W');
                        $video_prop['from_ip'] = "'".get_client_ip()."'";
                        $video_prop['family_id'] = intval($user_redis->getOne_db($to_user_id, 'family_id'));
