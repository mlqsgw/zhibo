 ...
 * */
    ...
    public function goods_inventory(){
        ...
        foreach($shop_info as $key => $value){
            $goods_info = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."goods WHERE inventory>=".$value['number']." and is_effect=1 and id=".$value['goods_id']);
            if(!$goods_info){
                $goods_name = $GLOBALS['db']->getOne("SELECT name FROM ".DB_PREFIX."goods WHERE is_effect=1 and id=".$value['goods_id']);
                $root['status'] = 0;
                $root['error'] = '库存不足。商品:'."$goods_name".'';
                break;
            }else{
                $root['status'] = 1;
            }
