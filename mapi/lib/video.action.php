...
//			$root['status'] = 1;
	...
	private function mix_stream2($room_id,$to_user_id){
		...
		if ($video['video_type'] == 1 && !empty($qcloud_security_key)) {
			...
			//获得连麦观众的列表，最多取最新3个
			$sql = "select user_id,play_rtmp,play_rtmp,channelid from ".DB_PREFIX."video_lianmai where stop_time = 0 and video_id =".video_id ." order by start_time desc limit 3";
			$list = $GLOBALS['db']->getAll($sql,true,true);
			...
			if ($total > 0) {
				...
				foreach ( $list as $k => $v )
				{
					...
					$user['layout_params'] = $this->get_layout($total, $image_layer,$m_config['qcloud_bizid']);
