...
class PropAction extends CommonAction{
    public function index()
    {
        ...
        if($_REQUEST['type']!=''&&intval($_REQUEST['type'])!=-1)
        {
 ...
 */	
	...
	public function view(){
	    $id = intval($_REQUEST ['id']);
	    $video_prop_where = 'prop_id = ' . $id;
	    //当前的对应月
	    $new_month = date('Ym');
	    $video_prop_table = "fanwe_video_prop_". $new_month;
	    //搜索时间条件处理
	    ...
	    if($begin_time){
	        ...
	        $video_prop_table = "fanwe_video_prop_". $begin_month;
	        $begin_month = str_replace('-', '', substr($begin_time, 0,8));
	        $video_prop_where .= ' and create_time>=' . strtotime($begin_time);
	    }
	    if($end_time){
	        if(!$begin_time){
	            $video_prop_table = "fanwe_video_prop_". $begin_month;
	            ...
	            $video_prop_table = "fanwe_video_prop_". $begin_month;
	            $video_prop_where .= ' and create_time<=' . strtotime($end_time);
	        }
	    }
	    //获取礼物
        $model = M('user');
        $sql = 'SELECT to_user_id,SUM(total_ticket) as total_ticket from '.$video_prop_table. ' where ' . $video_prop_where;
        $sql .= ' GROUP BY to_user_id ORDER BY total_ticket DESC';
        ...
        {
            $list[$key]['id'] = $i;
            $user_nick_name =$model->table($user_table)->where('id = ' . $row['to_user_id'])->field('nick_name')->find();
            $list[$key]['nick_name'] = $user_nick_name['nick_name'];
            $i++;
        }
