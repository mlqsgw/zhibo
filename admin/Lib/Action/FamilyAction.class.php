...
//        $family = $model->where($where)->field($field)->find();
        ...
        $field = 'f.*,j.create_time as time,u.nick_name,u.id,u.head_image,j.family_id,j.`status`,f.name';
        $count = $model->table($table)->where($where)->count();
        $p     = new Page($count, $listRows = 20);
        $family = $model->table($table)->where($where)->field($field)->order('f.id desc')->limit($p->firstRow . ',' . $p->listRows)->select();
        if (!isset($_REQUEST['status'])) {
            $_REQUEST['status'] = -1;
        }
        if ($_REQUEST['status'] != -1) {
        ...
        foreach ($family as $key => $value) {
            $list[$key]['time'] = to_date($value['time']);
