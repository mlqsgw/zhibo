...
class VideoPlaybackAction extends CommonAction{
            ...
            foreach($rel_data as $data)
            {
            	if ($data['live_in'] == 0&&(($data['is_live_pay']==1&&$data['live_pay_type'] != 0)||$data['is_live_pay']==0)){
                    ...
                    if ($ret['total_count'] > 0){
                    	if ($data['destroy_group_status'] != 1){
                    		//重新创建聊天组
	                    	...
	                    	if ($ret['ActionStatus'] == 'OK'){
	                    		...
	                    		$result['status'] = 1;
	                    		$result['info'] = '创建新群组失败:'.$ret['ErrorCode'].$ret['ErrorInfo'];
	                    		ajax_return($result);
	                    	}
                    	}
    ...
    public function insert_tecent_video(){
        ...
        if($user_id == 0)
        {
            ajax_return(array('status'=>'0','error'=>'请输入用户id'));
        }
        if(!check_empty($title))
        {
            ajax_return(array('status'=>'0','error'=>'请输入话题'));
        }
        if(!check_empty($video_vid))
        {
            ajax_return(array('status'=>'0','error'=>'请添加文件'));
        }
